...
function main() {
  ...
  // Chapter dividers — this edition carries no spreads, so instead assert the
  // five chapter dividers are present (one per section).
  const dividers = pages.filter((p) => p.kind === "divider");
  if (dividers.length !== 5) {
    errors.push(`expected 5 chapter dividers, got ${dividers.length}`);
  }
  const spreads = pages.filter((p) => p.kind === "spread");
  if (spreads.length !== 0) {
    errors.push(`this edition has no spreads; found ${spreads.length}`);
  }
