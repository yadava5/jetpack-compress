/**
 * Exports the running jetpack-compress System Card booklet
 * (http://localhost:5182) to a saddle-stitch PDF via puppeteer (one
 * 8.75×11.25in sheet per manifest page).
 *
 ...
 * page size for all 24 pages while still honoring bleed on the 7 full-bleed
 * pages (2 covers + 5 dividers). Non-bleed pages carry an internal 0.125in
...
const OUT_PATH = process.env.BOOKLET_OUT
  ...
  : resolve(OUT_DIR, "jetpack-compress-system-card.pdf");
